...
class Solution {
    public:
        int minTimeToReach(vector<vector<int>>& moveTime) {
            ...
            pendingMoves.push({0, 0, 0, 1});
            moveTime[0][0] = -1;
            while (!pendingMoves.empty()) {
                int row = pendingMoves.top()[1], col = pendingMoves.top()[2], val = pendingMoves.top()[0], alternateTime = pendingMoves.top()[3];
                pendingMoves.pop();
                if (row == n - 1 && col == m -1)
                    return val;
                for (auto [moveR, moveC] : moves) {
                    ...
                    if (moveTime[adjR][adjC] == -1)
                        continue;
                    pendingMoves.push({max(moveTime[adjR][adjC], val) + alternateTime, adjR, adjC, alternateTime == 1 ? 2 : 1});  
